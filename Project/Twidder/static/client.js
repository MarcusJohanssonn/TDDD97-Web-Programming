...
function forgotPW() {
  var feedback = document.getElementById("feedbackArea");
  var forgottenmail = document.getElementById("reemail").value;
  var forgottenmail = {'email': forgottenmail};
  console.log(forgottenmail);
  XMLrequest('POST', 'forgotpw', forgottenmail, function(response) {
    if (response.success) {
      feedback.innerHTML = response.message;
    ...
    document.getElementById("form_rest_pw").reset();
  })
}
    ...
    XMLrequest("GET", "messagetoken", "", function(response) {
      ...
      if (response.success) {
          ...
          if(arrayOfPosts.length > 0) {
            for (var i = arrayOfPosts.length-1; i > 0; i--) {
...
function dataChart() {
  ...
  var totalReg = 0;
  var messageOnPage = 0;
  var onlineRightNow = 0;
  var totSent = 0;
  XMLrequest('GET', 'data/', "", function(response) {
    console.log("first");
    if (response.success) {
        console.log("IM HERE");
        //  arrayOfPosts = (response.messages);
        console.log(response.messagenr);
        console.log(response.onlineusers);
        console.log(response.total);
        totalReg = response.total;
        messageOnPage = response.messagenr;
        onlineRightNow = response.onlineusers;
        totsent = response.sent;
        var chart = new Chart(ctx, {
            // The type of chart we want to create
            type: 'bar',
            // The data for our dataset
            data: {
                labels: ['NrOfPosts',  'Sent Posts', 'Total registered accounts', 'Online ATM'],
                datasets: [{
                    label: 'Data of your twidder',
                    backgroundColor: 'rgb(255, 99, 132)',
                    borderColor: 'rgb(255, 99, 132)',
                    data: [messageOnPage, totsent, totalReg, onlineRightNow]
                }]
            },
